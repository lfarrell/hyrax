RSpec.describe 'hyrax/my/collections/_list_collections.html.erb', type: :view do
  ...
  context "check for admin set" do
    ...
    it 'the line item displays the work and its actions' do
      expect(rendered).to include 'title="Delete administrative set" data-totalitems="0" data-membership="true" data-hasaccess="true"'
      expect(rendered).to have_selector("tr#document_#{id}")
      expect(rendered).to have_link 'AdminSet Title', href: '#'
      expect(rendered).to have_link 'Edit administrative set', href: hyrax.admin_admin_set_path(id)
      expect(rendered).to have_link 'Delete administrative set', href: hyrax.admin_admin_set_path(id)
